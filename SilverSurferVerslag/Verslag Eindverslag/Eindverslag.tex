...
\label{sec:algo}
 == veelhoek == %
\subsection{Rijden van een veelhoek} % 3 ?
...
Ons voorstel is gebaseerd op het spel bomberman van Hudson Soft. (Hoofdwebsite van hudson entertainment is niet meer beschikbaar op 12/12/2012 \mbox{$[http://en.wikipedia.org/wiki/Bomberman]$})

\subsection{Doelstelling}
De robots proberen elkaar uit te schakelen.

\subsection{Probleemstelling}
\begin{itemize}
\item
Het speelveld is een doolhof geconstrueerd op dezelfde manier als de doolhoven tijdens semester \'e\'en. Het doolhof bestaat het best uit korte rechte stukken met veel zijwegen door het spelmechanisme.
...
\item
De robots proberen elkaar uit te schakelen door het plaatsen van bommen, die na een bepaalde tijd ontploffen. Een bom is een obstakel waar de robots niet door kunnen. Als een bom ontploft dan gaat alles kapot dat zich in een rechte lijn binnen een bepaalde afstand van de bom bevindt (enkel horizontaal en verticaal). Explosies gaan dus niet diagonaal en ook niet door muren. Om te voorkomen dat robots gewoon heel de tijd bommen achterlaten bij het rijden is het aantal bommen dat tegelijk actief kan zijn twee, je kan dus pas een derde bom leggen als de eerste van die twee ontploft is.

\item
Er zijn ook obstakels (virtuele) die de robot kan vernietigen met een bom, dan wordt het vakje waarop het obstakel stond vrij gemaakt. En kunnen de robots hier ook door.

\item
Er zijn ook twee 'power-ups' die respectievelijk de bommen van een robot sterker maken of de robot beschermen. Als je over een power-up rijdt dan wordt die automatisch van toepassing en verdwijnt die ook. Power-ups kunnen natuurlijk ook vernietigt worden door de explosie van  een bom. De  power-up die bommen sterker maakt, heeft als resultaat dat de afstand waarover de explosie gebeurt vergroot. De power-up die bescherming biedt, zorgt ervoor dat de robot \'e\'en explosie kan overleven zonder schade.
...
\begin{figure}[h!]
  \caption{Bomberman}
  \centering
    \includegraphics[width=0.5\textwidth]{bomberman.png}
  \label{fig:bomberman}
\end{figure}
...
\item
Robot 2 (R2) heeft niet zoveel geluk, hij is geraakt en  dus vernietigt door de bom naast hem. Deze bom kan hij zelf geplaatst hebben of kan door een tegenstander geplaatst zijn.
