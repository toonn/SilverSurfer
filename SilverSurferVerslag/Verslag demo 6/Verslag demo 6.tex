...
De robot is voorzien van een lichtsensor, een infraroodsensor en een ultrasone sensor. Verder heeft de robot een strip klittenband achteraan op een lange schep waarmee hij het voorwerp kan oprapen en eventueel een wip kan openen.
...
%TODO wat gaat er komen in welke paragrafen?
...
\item \textbf{Figuur~\ref{fig:robotBouw}}. De wielen staan verder uit elkaar. De NXT staat schuin gemonteerd om een zo compact mogelijk ontwerp te krijgen. Zo kwam aan de achterkant opnieuw genoeg plaats om een schep te monteren.
...
Deze laatste opstelling is de huidige opstelling. Voorlopig werden hier nog geen nadelen aan ondervonden.\\
...
Wanneer deze wip open is zal de robot hem oversteken. Wanneer de wip gesloten is kan de robot de wip openen met behulp van de schep. Deze functie kan ook worden uitgezet. In dat geval zal de robot naar een andere wip rijden of blijven wachten voor de wip tot een andere robot de wip opent.

\subsection{Het openen van de wip} 
...
\item tegels waarvan de vier zijden al gekend zijn en die niet van het type `straight' zijn worden niet meer bezocht, deze kunnen immers geen barcode bevatten waardoor het geen extra informatie levert de tegel te bezoeken.
\end{enumerate}
...
\subsection{Het oprapen van een voorwerp}
...
Wanneer een robot zijn voorwerp gevonden heeft, weet hij bij welk team hij hoort. Hij stuurt vanaf dan periodisch zijn map door en kijkt regelmatig of hij een nieuwe map van zijn teamgenoot ontving.
...
\section{Besluit}
\label{sec:Besluit}
De bouw van de robot bevat een lange schep, voorzien van klittenband. De schep kan via een motor naar beneden worden gelaten. Eventueel kan deze gebruikt worden om een wip te laten zakken. Dit wordt alleen gebruikt wanneer de robot geen andere uitweg meer heeft. De lichtsensor is gemonteerd op een hefboomsysteem dat omhoog klapt wanneer het de wip raakt. Zo geraakt de robot makkelijk de wip op.\\
