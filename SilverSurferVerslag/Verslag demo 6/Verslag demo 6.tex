...
De robot is voorzien van een lichtsensor, een infraroodsensor en een ultrasone sensor. Verder heeft de robot een strip klittenband achteraan op een schep waarmee hij het voorwerp kan oprapen.
...
\label{ssec:FysB}
Een infraroodsensor werd gemonteerd bovenop de robot. Doordat er een infraroodbal onder een wip ligt, kan de robot bepalen of de wip naar beneden of naar boven staat. In het geval dat de wip naar beneden staat wordt het infrarood licht immers geblokkeerd.\\

Het voorwerp kan op verschillende manieren worden opgeraapt. Volgende opstellingen werden gebouwd en getest:

\begin{enumerate}
\item \textbf{Figuur~\ref{fig:robotOud1}}. De robot heft het voorwerp op met behulp van een extra motor. Deze opstelling maakt de robot te lang waardoor hij moeilijk kon draaien zonder tegen een muur te botsen. Er werd besloten de schep vooraan te plaatsen.
...
\item \textbf{Figuur~\ref{fig:robotBouw}}. De robot neemt nog steeds met behulp van klittenband het voorwerp op. De schep bevind zich nu op het uiteinde van een steel. Deze steel wordt gebruikt om de wip naar beneden te kantelen. Het voorwerp wordt dan helemaal mee tot boven opgetild.  
\end{enumerate}


Het vorig ontwerp van de robot is aangepast. Uit de vorige demo bleek dat de robot bij het oprijden en afrijden van de wip heel schokkerig bewoog, ook wanneer de robot draaide of sneller wou rijden. Voor deze demo hebben we de robot helemaal terug opnieuw in elkaar gestoken. De wielen zijn verder uit elkaar gezet zodat de robot breder wordt. De NXT is schuin erop gezet omdat het zo goed uit kwam om te monteren. Het bijkomend voordeel dat we hierbij hadden, was dat er dan aan de achterkant genoeg plaats was om een schep voor het voorwerp te monteren. \\
...
De infraroodsensor bestaat uit vijf onafhankelijke sensoren. De robot maakt enkel gebruik van de middelste sensor omdat de wip zich altijd voor de robot zal bevinden. De infraroodsensor geeft gemiddeld een waarde van $1$ , $2$ of $3$ tenzij hij een infraroodbal detecteert. In dat geval geeft hij gemiddeld een waarde rond de $120$.
...
Bij het oprapen van het voorwerp rijdt de robot achteruit tot de schep tegen de muur komt. Met de druksensor kan het slagen van deze actie gecontroleerd worden. Het is echter niet haalbaar een druksensor op de lange schep te plaatsen. Daarom wordt er op de robot geen druksensor gemonteerd.
...
Het is niet mogelijk om de robot perfect 100\% nauwkeurig te doen rijden. De motoren kunnen immers maar tot op een bepaalde nauwkeurigheid ingesteld worden. Om de besturing toch betrouwbaar te maken is het nodig de robot af en toe te aligneren. Dit gebeurt bij elke tegel. De robot kan dit snel genoeg doen dus zorgt dit niet voor al te veel vertraging.
...
Wanneer de volledige projectie bepaald is, kan deze elke keer gebruikt worden wanneer een nieuwe doolhof ontvangen wordt. Ze hoeft niet opnieuw berekend te worden.
...
Om naar de juiste plek te gaan gebruikt de robot het kortste pad algoritme. Elke keer wanneer het kortste pad berekend wordt, gaat de robot 1 tegel vooruit. Nadien vraagt het terug de positie van de andere robot om opnieuw het kortste pad te berekenen. Door na elke tegel terug het nieuwste kortste pad te berekenen, zorgt de robot ervoor dat hij up to date blijft over de positie van de andere robot die ondertussen ook naderbij komt.
%TODO Gerlinde updaten

\subsection{Andere robots}
\subsubsection{Robot-detectie sensor}
\label{ssec:AlgoCollision}

De hybride sensor wordt door ons team niet ge"implementeerd. De hybride sensor maakt gebruikt van de wereld simulator en de kennis van de positie die de andere robots doorsturen. De posities van de andere robots moeten dan correct zijn voordat de wereld simulator correct is. Wij hebben ervoor gekozen om de robot robuuster te maken en onafhankelijk van andere robots. Dit is de reden waarom de robot-detectie sensor (of hybride sensor) niet gebruikt wordt door de robot. Voor robot detectie gaat de robot elke muur wantrouwen. Het kan dus zijn dat de robot een muur detecteert maar dat dit eigenlijk een robot is. Als een robot het doolhof verkent, zal hij elke keer weer naar muren zoeken. Een andere robot zal dus als een muur gedetecteerd worden. Als de robot op een ander tijdstip terug op het vakje komt en de robot is er niet meer, dan detecteert hij bij het zoeken van de muren geen muur meer en veranderd dit in zijn geheugen. \\

Wanneer een robot een opening ziet, is de robot wel zeker dat daar geen muur is.
Als er gesimuleerde robots in het spel zijn, wordt er wel de informatie van de wereldsimulator gebruikt om de afstandssensor juist te simuleren (je moet de positie van andere robots weten om de afstand tot die robots te simuleren). Dit is dus ook maar benaderend want je hebt alleen 'matrixpositions' en geen absolute positions, je doet dus alsof ze op het midden van een tegel staan.\\


% == SOFTWARE == %
\section{Software}
\label{sec:Softw}
De software bestaat uit twee delen: een project dat op de NXT van de robot loopt en een project dat op de computer loopt (sectie~\ref{ssec:Sdesign}). Alles wordt aangestuurd via de \textit{Graphical User Interface (GUI)} (sectie~\ref{ssec:GUI}). Deze laat toe de robot te besturen en de reacties van de robot weer te geven. Robots kunnen met elkaar communiceren via RabbitMQ (sectie~\ref{ssec:RabbMQ}). Via de GUI kunnen ook virtuele robots aangestuurd worden i.e. gesimuleerde robots (sectie~\ref{ssec:Sim}). Tijdens het verkennen wordt een map opgeslagen van de wereld (sectie~\ref{ssec:Mapping}).\\
...
Figuren~\ref{fig:GUI1} en~\ref{fig:GUI2} tonen de GUI bij verschillende instellingen. De eerste figuur toont de GUI met de verschillende robots met elk hun eigen doolhof, voorwerpen en barcodes erin. Elke robot heeft initieel een eigen kleur. Deze kleur komt overeen met het voorwerp dat de robot moet zoeken en met de rand van de kader waarin de robot zich bevindt. Wanneer een robot zijn voorwerp gevonden heeft, verandert zijn kleur in de teamkleur. Dit is te zien in de eerste groene kader van figuur \ref{GUI1}. Hierbij heeft de robot zijn voorwerp gevonden maar hij weet nog niet wie zijn teamgenoot is. Wanneer zijn teamgenoot ook zijn voorwerp gevonden heeft veranderd ook zijn kleur in de teamkleur en mappen de beide robots hun doolhof. Dit doolhof is te zien rechts van de kader van de beide robots. Dit is te zien in de tweede figuur. De robot met de blauwe kader en de robot met de groene kader hebben elk hun voorwerp gevonden en hebben dezelfde teamkleur. Beiden hebben ook al hun doolhof gemapt. Op het einde hebben de robots van hetzelfde duo dezelfde kleur.\\
De \textit{OverallViewport} staat helemaal linksboven. Hierin worden alle robots weergegeven, de wip, de voorwerpen en de barcodes. Het voorwerp heeft dezelfde kleur als de robot die het moet oppakken.\\
...
Het aantal \textit{UnitViewports} hangt af van de situatie. Dit kan makkelijk worden ingesteld via de menubalk. Het is ook mogelijk extra panelen te laten verschijnen die sensorwaarden op een grafiek en in cijfers weergeven. Ook het paneel om de robot handmatig te besturen kan tevoorschijn gehaald wordt via de menubar. Dit is te zien in figuur~\ref{fig:GUI2}. \\
Links worden de kleuren van de verschillende robots weergegeven en de teamkleuren. Extra info zoals de sensorwaarden en de positie staat eronder beschreven. Beneden de \textit{Vieuwports} staan de grafieken voor de sensorwaarden van de  lichtsensor, de ultrasone sensor en de infraroodsensor. 
...
\section{Besluit}
\label{sec:Besluit}
De bouw van de robot wordt uitgebreid met een infraroodsensor. De schep wordt aangepast en achteraan geplaatst. De schep wordt ook van klittenband voorzien. De schep is nu zodanig lang gemaakt dat de schep via een motor de wip naar beneden kan halen. Dit wordt alleen gebruikt wanneer de robot vast zit in het doolhof. De lichtsensor is gemonteerd op een hefboomsysteem dat omhoog klapt wanneer het de wip raakt. Zo geraakt de robot makkelijk de wip op.\\
...
De simulator van de robot was wel in orde. Het was mogelijk om 4 robots te simuleren. Het enige minpuntje zat hem in de visualisatie van het pad. Er zaten discontinu"iteiten in waardoor er schokken zaten in de visualisatie van het pad. \\
...
\item \textit{\ref{ssec:GUI} GUI:} nieuwe tekst aangepast aan de nieuwe GUI en nieuwe figuren.
\end{itemize}
