...
public abstract class AbstractPilot implements PilotInterface {
	...
	public void activateTeamPilot(String teamMemberName) {
		...
		new Timer(1000 / updateTilesAndPositionFPS, updateTilesAndPosition)
				.start();
	}
	...
	private void updateTilesAndPosition() {
		...
		try {
			...
			getCenter().updatePosition((int) getPosition().x,
					(int) getPosition().y, (int) getAngle());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void startLookingYourTeammate() {
		...
		if ((tilesAway == 1 || tilesAway == 0)
			...
			System.out
					.println(mapGraphConstructed
							.convertPoint(getTeammatePosition()).x
							+ " "
							+ mapGraphConstructed
									.convertPoint(getTeammatePosition()).y);
			System.out.println(getMatrixPosition().x + " "
					+ getMatrixPosition().y);
			won();
			return;
		} else if (tilesAway <= 4) {
