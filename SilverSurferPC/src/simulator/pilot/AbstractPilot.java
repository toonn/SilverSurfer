...
public abstract class AbstractPilot implements PilotInterface {
    ...
    public void travel(final double distance) {
        ...
        for (int i = 1; i <= Math.abs(distance); i++) {
            ...
            if (readBarcodes
                    && this instanceof SimulationPilot
                    && getMapGraphConstructed().getTile(getMatrixPosition()) != null
                    && !(getMapGraphConstructed().getTile(getMatrixPosition())
            ...
            if (onFlipEdge()) {
                int seesawValue = getMapGraphConstructed()
                        .getTile(getMatrixPosition()).getContent().getValue();
                getMapGraphConstructed().flipSeesaw(seesawValue);
                System.out.println("Robot " + getTeamNumber()
                        + ": flip seesaw " + seesawValue);
    ...
    private boolean onFlipEdge() {
        if (getMapGraphConstructed().getTile(getMatrixPosition()).getContent() instanceof Seesaw) {
            ...
            // you are standing right in front of an edge; if this edge is a
            // flip, return true
            if ((position < 1 && getOrientation() == equivalentOrientation)
                    || (position > sizeTile() - 1 && getOrientation() == equivalentOrientation
                            .getOppositeOrientation()))
                return getMapGraphConstructed().getTile(getMatrixPosition())
                        .getEdge(getOrientation()).getObstruction() == Obstruction.SEESAW_FLIP;
            // you are standing right across of an edge; if this edge is a flip,
            // return true
            else if ((position < 1 && getOrientation() == equivalentOrientation
                    .getOppositeOrientation())
                    || (position > sizeTile() - 1 && getOrientation() == equivalentOrientation))
                return getMapGraphConstructed().getTile(getMatrixPosition())
                        .getEdge(getOrientation().getOppositeOrientation())
                        .getObstruction() == Obstruction.SEESAW_FLIP;
            else
                return false;
        } else
            return false;
