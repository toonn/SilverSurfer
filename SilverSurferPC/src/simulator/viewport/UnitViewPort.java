...
public class UnitViewPort extends DummyViewPort {
    ...
    private void paintBeamComponent(final Graphics graph) {
        for (AbstractPilot pilot : pilots) {
            ...
            sonarArc.setArc(pilot.getPosition().getX() - correctedUSDistance,
                    pilot.getPosition().getY() - correctedUSDistance,
                    2 * correctedUSDistance, 2 * correctedUSDistance,
                    360 - pilot.getAngle() - 15, 30, Arc2D.PIE);
            ...
            if (pilot.getUltraSensorValue() > 200
                ...
                graph.setColor(new Color(133, 211, 249)); // Light blue when not
                                                          // trusted (< 20 || >
                                                          // 200)
            } else {
                graph.setColor(new Color(12, 24, 244)); // Dark blue when
                                                        // trusted (>= 20 && <=
                                                        // 200)
            }
            ...
            for (MapGraph mapGraph : getAllMapGraphs()) {
                for (Tile tile : mapGraph.getTiles()) {
                    Rectangle2D checkHighlight = new Rectangle2D.Double(tile
                            .getPosition().getX() * 40, tile.getPosition()
                            .getY() * 40, (int) getSizeTile(),
                            (int) getSizeTile());
