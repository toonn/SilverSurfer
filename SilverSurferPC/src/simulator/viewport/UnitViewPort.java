...
public class UnitViewPort extends DummyViewPort {
    ...
    /*
     * public void moveRobot(double x, double y, final double degrees) { x = x *
     * scalingfactor; y = y * scalingfactor;
    ...
    private void updatePathComponent() {
        for (final PilotInterface pilot : pilots)
            if (pilot.getPosition().getX() != pathCoordinates.get(
                    pathCoordinates.size() - 1).getX()
                    || pilot.getPosition().getY() != pathCoordinates.get(
                            pathCoordinates.size() - 1).getY())
                addPathPoint(pilot.getPosition().getX(), pilot.getPosition()
                        .getY());
    }
    ...
    private void paintPathComponent(final Graphics graph) {
        ...
        for (int i = 0; i < pathCoordinates.size() - 1; i++)
            g2.draw(new Line2D.Double(pathCoordinates.get(i), pathCoordinates
                    .get(i + 1)));
    ...
    //
    // //updateUndergroundCircle(pilot.getLightSensorCoordinates()[0] *
    // scalingfactor, pilot.getLightSensorCoordinates()[1] * scalingfactor,
    // pilot.getLightSensorValue());
    ...
    // undergroundCircle.setFrame(pilot.getPosition().getX() - (diam / 2),
    // pilot.getPosition().getY() - (diam / 2), diam, diam);
    // }
    ...
    private void paintBeamComponent(final Graphics graph) {
        for (AbstractPilot pilot : pilots) {
            ...
            // (overallviewport, dummyviewport...) --- WAAROM?
            updateArc(pilot.getPosition().getX(), pilot.getPosition().getY(),
                    pilot.getAngle(), pilot.getUltraSensorValue());
