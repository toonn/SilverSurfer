...
public class ShortestPath {
    ...
    public ShortestPath(Communicator communicator, Tile startTile,
            Tile endTile, Vector<Tile> tiles) {
    ...
    public void goShortestPath() {
        ...
        for (int i = 0; i < tilesPath.size() - 1; i++) {
            int[] ar = new int[2];
            ar[0] = ((Tile) tilesPath.get(i + 1)).getxCoordinate()
                    - ((Tile) tilesPath.get(i)).getxCoordinate();
            ar[1] = ((Tile) tilesPath.get(i + 1)).getyCoordinate()
                    - ((Tile) tilesPath.get(i)).getyCoordinate();
            Orientation orientation = Orientation.getOrientationOfArray(ar);
            try {
                if (tilesPath.size() - i > 2)
                    communicator.sendCommand(
                            Command.STOP_READING_BARCODES);
                else
                    communicator.sendCommand(
                            Command.START_READING_BARCODES);
    ...
    private void setHeuristics() {
        for (Tile tile : tiles) {
            ...
            int heuristic = Math.abs(xNextTile - tile.getxCoordinate())
    ...
    private void deleteSuperfluousTiles() {
        if (tilesPath.size() > 1) {
            for (int i = tilesPath.size() - 2; i != 0; i--) {
                if ((((Tile) tilesPath.get(i)).getCost() != ((Tile) tilesPath
                        .get(i + 1)).getCost() - 1)
                        || !((Tile) tilesPath.get(i))
                                .areNeighbours((Tile) tilesPath.get(i + 1))) {
                    tilesPath.remove(i);
                }
            }
        }
