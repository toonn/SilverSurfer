    ...
    private void algorithm(Tile currentTile) {
        ...
        for (Object neighbourTile : currentTile.getReachableNeighbours()) {
            if (neighbourTile != null
                ...
                /*
                 * int i = 0; for(Object neighbour: ((Tile)
                 * neighbourTile).getAllNeighbours()) { if(neighbour != null &&
                 * (((Tile) neighbour).isMarkedExploreMaze())) i++; } if(i == 4
                 * && ((Tile)neighbourTile).getAmountOfWalls() > 2) { ((Tile)
                 * neighbourTile).setMarkingExploreMaze(true);
        ...
        if (currentTile.getEdge(currentOrientation).isPassable()
                && currentTile.getEdge(currentOrientation).getNeighbour(
                        currentTile) != null
                && queue.contains(currentTile.getEdge(currentOrientation)
                        .getNeighbour(currentTile)))
            nextTile = currentTile.getEdge(currentOrientation).getNeighbour(
                    currentTile);
        else if (currentTile.getEdge(
                currentOrientation.getOtherOrientationCorner()).isPassable()
                && currentTile.getEdge(
                        currentOrientation.getOtherOrientationCorner())
                        .getNeighbour(currentTile) != null
                && queue.contains(currentTile.getEdge(
                        currentOrientation.getOtherOrientationCorner())
                        .getNeighbour(currentTile)))
            nextTile = currentTile.getEdge(
                    currentOrientation.getOtherOrientationCorner())
                    .getNeighbour(currentTile);
        else if (currentTile.getEdge(
                currentOrientation.getOtherOrientationCorner()
                        .getOppositeOrientation()).isPassable()
                && currentTile.getEdge(
                        currentOrientation.getOtherOrientationCorner()
                                .getOppositeOrientation()).getNeighbour(
                        currentTile) != null
                && queue.contains(currentTile.getEdge(
                        currentOrientation.getOtherOrientationCorner()
                                .getOppositeOrientation()).getNeighbour(
            ...
            nextTile = currentTile.getEdge(
                ...
                currentOrientation.getOppositeOrientation()).isPassable()
                && currentTile.getEdge(
