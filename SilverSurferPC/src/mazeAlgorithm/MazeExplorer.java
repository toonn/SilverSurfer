...
//    }
    ...
    private void algorithm(Tile currentTile) {
    	...
    	currentOrientation = gui.getCommunicator()
...
//        }
    ...
    /**
     * checkt of alle neighbourTiles al gecheckt zijn,
     * indien ja niet meer nodig om nextTile nog te checken.
     */
    ...
    /**
     * Kijkt naar elke neighbourTile van de currentTile:
     * wanneer nog niet explored en geen obstruction wordt een tile gezet.
     */
    ...
    private Tile getPriorityNextTile(Tile currentTile){
        ...
        if (isGoodNextTile(currentTile, currentOrientation))
            return currentTile.getEdge(currentOrientation).getNeighbour(
                    currentTile);
        else if (isGoodNextTile(currentTile,currentOrientation.getOtherOrientationCorner()))
            ...
            return currentTile.getEdge(
                    currentOrientation.getOtherOrientationCorner()
                            .getOppositeOrientation())
                    .getNeighbour(currentTile);
        else if (isGoodNextTile(currentTile, currentOrientation.getOppositeOrientation())){
            return currentTile.getEdge(
    ...
    private boolean isGoodNextTile(Tile currentTile, Orientation orientation){
    	return currentTile.getEdge(orientation).isPassable()
                && currentTile.getEdge(orientation)
